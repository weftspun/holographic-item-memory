...
**Status:** Accepted · 2026-07-14 · **Supersedes** [Switch the recommender to RecGPT / FuXi-Linear over Residual FSQ semantic IDs](20260713-recgpt-fuxi-linear-residual-fsq.md)

## Context and problem statement

The prior decision ([20260713](20260713-recgpt-fuxi-linear-residual-fsq.md), Accepted) added the
RecGPT / FuXi-Linear generative recommender over residual FSQ semantic IDs and **kept
`Holo.Core.Memory` (the training-free HRR memory) as an apples-to-apples baseline** consuming
identical IDs.
...
1. **Remove the HRR recommender from the Elixir tree** — delete `Holo.Core.Memory` and
   `Holo.Core.HRR` (and their tests / golden fixture). FuXi-Linear (`Holo.Adapters.Serve` over
   `Holo.Core.{FuxiLinearInference*, Decode, Trie}`) is the sole recommender.

2. **Rehome the semantic-ID contract** (`tokens_per_item/0`, `codebook_size/0`, `valid_id?/1`) from
   `Holo.Core.Memory` into `Holo.Core.ResidualFSQ` — the codec that emits the IDs — as the single
   source of truth every consumer (CLI, store adapters, `Serve`) shares.

3. **Rewire the persistence read path.** `Holo.Ports.ItemSource.load_memory/3` (which built a
   `Holo.Core.Memory`) is replaced by `load_catalog/2`, returning `{token_id_list, item_ids}` — the
   catalog `Holo.Adapters.Serve` consumes (index = model item index).

4. **Keep CockroachDB.** Persistence stays the embedded `Holo.Adapters.CockroachStore` behind the
...
5. **CLI `recommend` now serves through the model.** `holo recommend <id…> --checkpoint <dir>` loads
...
- **Positive:** one recommender, one ID contract, less surface. The Lean `HoloModel` codec proofs
...
- **Negative / trade-offs:** the training-free floor is gone — recommendation now requires a trained
  checkpoint, so `holo recommend` is inert until a model is trained on a GPU host (gate-clean data:
...
- The Lean HRR **phase-algebra** proofs (`bind_comm`, …) remain in `formal/HoloModel.lean` as formal
