...
defmodule Holo.Adapters.AxonTrainer do
  @moduledoc """
  Training glue for the FuXi-Linear recommender.

  Rewired from RecGPT.Adapters.AxonTrainer, which drove the *non-FuXi*
  `Core.Inference` baseline — this repo is decoder-only FuXi-Linear, so the
  forward pass here is `Holo.Core.FuxiLinearInference.forward_full_sequence/5`
  (full-sequence logits `(batch, seq, 4097)`) and the objective is
  `Holo.Core.Training.loss_shifted_ce/2` plus a Multi-Token-Prediction term
  ...
  builds batches via `Holo.Core.Training.build_train_batch/5`. Gradient descent
  ...
  alias Holo.Core.{FuxiLinearInference, Training}
  ...
  `Holo.Core.Training.build_train_batch/5` returns.
