...
defmodule Holo.Adapters.NpyCheckpointSource do
  @moduledoc """
  Load RecGPT checkpoint from an export directory (manifest.json + .npy files).

  Export dir can be produced by:
  - `Holo.Adapters.NpyCheckpointSink.write_export/2` or `mix holo.export_ckpt --from-export DIR --out DIR`
  - `mix holo.export_ckpt --from-pt path.pt --out DIR` (requires Python + torch)
  ...
  Returns a map of key => Nx.Tensor. The inference model (Holo.Core.Inference) expects
  ...
  When `config :holographic_item_memory, :ckpt_expected_sha256` or `HOLO_CKPT_SHA256` is set,
  load_from_export/1 verifies the checkpoint hash before loading. Compute the hash with
  `mix holo.ckpt_sha256 --ckpt path`.
  """

  @behaviour Holo.Ports.CheckpointSource
  ...
  Verifies SHA256 when config :holographic_item_memory, :ckpt_expected_sha256 or HOLO_CKPT_SHA256 is set.
  """
  @impl Holo.Ports.CheckpointSource
    ...
    if expected && expected != "" do
      ...
      if actual != expected do
        raise "Checkpoint SHA256 mismatch: expected #{expected}, got #{actual}. " <>
                "Run mix holo.ckpt_sha256 --ckpt #{export_dir} to get the correct hash."
      end
    end
  ...
  defp expected_ckpt_sha256 do
    System.get_env("HOLO_CKPT_SHA256") || Application.get_env(:holographic_item_memory, :ckpt_expected_sha256)
  end
