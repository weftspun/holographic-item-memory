  ...
  def evaluate(state, test_cases, opts \\ []) do
    ...
    recommend_fn =
      Keyword.get(opts, :recommend_fn) ||
        fn ctx, k -> Holo.Adapters.Serve.recommend(state, ctx, k) end

    recommend_batch_fn =
      Keyword.get(opts, :recommend_batch_fn) ||
        fn ctx_list, k -> Holo.Adapters.Serve.recommend_batch(state, ctx_list, k) end
