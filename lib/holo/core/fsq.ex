...
defmodule Holo.Core.FSQ do
  @moduledoc """
  Finite Scalar Quantization — **residual** variant, one stage.

  Port of the RecGPT `Core.FSQ` stage math (`bound` / `round_ste` /
  ...
  exactly `HoloModel.stageIndex c0 c1 c2 c3 = c0 + c1*8 + c2*64 + c3*512`, so
  ...
  (`{n, 4}`, `{n, s, 4}`, …). `Holo.Core.ResidualFSQ` iterates a stage over the
  ...
  # HoloModel.stageIndex's [1, 8, 64, 512] basis.
  @basis [1, 8, 64, 512]
  @codebook 4096
  ...
  @doc """
  Squash `z` into the representable per-dim range (RecGPT `bound`, verbatim math).
  ...
  Equal to `HoloModel.stageIndex` on the recovered digits.
  """
  ...
  `HoloModel.stage_roundtrip` (`idx % 8`, `idx / 8 % 8`, `idx / 64 % 8`,
  `idx / 512 % 8`).
  """
  ...
  `Holo.Core.ResidualFSQ.encode/2` consumes (`%{"project_in" => %{"kernel" => k,
  "bias" => b}}`).
