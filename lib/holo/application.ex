...
defmodule Holo.Application do
  @moduledoc """
  OTP application for `:holographic_item_memory`.
  ...
  `Holo.Adapters.CLI.main/0`, turning the binary into the `holo` CLI.
  ...
  `HOLO_CLI=0` to suppress it even in the standalone binary.
  """
  ...
  def start(_type, _args) do
    children =
      if run_cli?() do
        ...
        [Supervisor.child_spec({Task, fn -> Holo.Adapters.CLI.main() end}, restart: :temporary)]
      else
        []
      end

    Supervisor.start_link(children, strategy: :one_for_one, name: Holo.Supervisor)
  end

  defp run_cli? do
    System.get_env("HOLO_CLI") != "0" and System.get_env("__BURRITO") != nil
  end
end
