...
defmodule Recommender.Adapters.TrajectoryConvert do
  @moduledoc """
  License-gated trajectory ingestion: raw (gate-clean) datasets → per-user
  chronological item sequences for training / eval.

  Every ingest path passes through `Recommender.Core.Trajectories.LicenseGate`: a
  ...
  / non-Western) raise `Recommender.Core.Trajectories.LicenseError` before any bytes are
  read — there is no override. The chronological train/test split is delegated to
  `Recommender.Core.Trajectories.Split`.

  Primary gate-clean corpus: **open-ecommerce** (Amazon purchases, CC0):
  `user = "Survey ResponseID"`, `time = "Order Date"`, `item = "ASIN"`.
  """

  alias Recommender.Core.Trajectories.{LicenseGate, Split}

  NimbleCSV.define(Recommender.Adapters.TrajectoryConvert.CSV, separator: ",", escape: "\"")
  alias Recommender.Adapters.TrajectoryConvert.CSV
  ...
  def sequences(dataset_name, root_dir) do
    ...
    unless entry["english"] == true do
      raise Recommender.Core.Trajectories.LicenseError,
