...
defmodule Recommender.Adapters.NpyCheckpointSink do
  @moduledoc """
  Write model checkpoint to an export directory (manifest.json + .npy files)
  compatible with `Recommender.Adapters.NpyCheckpointSource.load_from_export/1`.

  Uses the `:npy` package to save each tensor. Use this to:
  - Save trained params from `Recommender.Adapters.AxonTrainer.run/3` to disk.
  - Re-export an existing export dir (load then write to another path).
  - Produce the same format that the former Python script `inspect_checkpoint.py --export` produced.
  """

  @behaviour Recommender.Ports.CheckpointSink
  ...
  """
  @impl Recommender.Ports.CheckpointSink
