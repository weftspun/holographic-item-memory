...
defmodule Recommender.Adapters.BumblebeeEmbedding do
  @moduledoc """
  Text → 768-d embeddings for generative training (MPNet, sentence-transformers/all-mpnet-base-v2).

  Uses Bumblebee with mean pooling, L2 normalization, and fixed sequence length 384 to align with
  sentence-transformers (SentenceTransformer.encode). Run `mix recommender.compare_embeddings` to check
  cosine similarity vs the dataset item_text_embeddings.npy.

  ## Usage

      serving = Recommender.Adapters.BumblebeeEmbedding.serving()
      result = Nx.Serving.run(serving, "Some market title Yes")
      result = Nx.Serving.run(serving, ["Text A", "Text B"])
      embeddings = Recommender.Adapters.BumblebeeEmbedding.encode_item_text_dict(%{0 => "Title A", 1 => "Title B"})
  """

  @behaviour Recommender.Ports.EmbeddingSource
  ...
  @doc """
  Builds the same string upstream generative uses for encoding: Python's str(dict).replace('{','').replace('}','').
  ...
  """
  @spec item_text(map()) :: String.t()
  def item_text(item) when is_map(item) do
    title =
  ...
  @doc "Loads the MPNet model and tokenizer, returns a text embedding serving. Cached in application env (`:embedding_serving`)."
  def serving do
    case Application.get_env(:residual_fsq_recommender, :embedding_serving) do
      nil ->
        serving = load_serving!()
        Application.put_env(:residual_fsq_recommender, :embedding_serving, serving)
        serving
  ...
  @impl Recommender.Ports.EmbeddingSource
