...
defmodule Recommender.Core.Decode do
  @moduledoc """
  Next-item prediction: beam search (default) or Multi-Token Prediction (MTP).

  - **Beam search**: Decodes 4 tokens (one item) over 4 steps; the t-th token waits for
  ...
  def beam_search_top_k_spmd(
    ...
    beam_width =
      case Keyword.get(opts, :beam_width_override) ||
             Application.get_env(:residual_fsq_recommender, :beam_width_override) do
