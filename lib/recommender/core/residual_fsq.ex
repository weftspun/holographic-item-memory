...
defmodule Recommender.Core.ResidualFSQ do
  @moduledoc """
  Residual FSQ driver: content embeddings → 4-token semantic IDs `[t0, t1, t2, t3]`.

  This is the ADR's core change from the archived elixir-sequential-recommendation reference, which used *grouped*
  ...
  Each `t_s` is a full 4096-code stage index (`Recommender.Core.FSQ`, levels `[8,8,8,8]`),
  ...
  and `RecommenderModel.itemKey_injective` certify.
  ...
  adapters, and `Recommender.Adapters.Serve` — shares.
  ...
  consumer (CLI, store adapters, `Recommender.Adapters.Serve`) shares.
  """
  ...
  catalog form the store adapters persist and `Recommender.Adapters.Serve` consumes.
  """
  ...
  mirror of `RecommenderModel.itemKey t0 t1 t2 t3 = t0 + t1*4096 + t2*4096^2 + t3*4096^3`.
