...
defmodule Recommender.Core.FuxiLinearInference do
  @moduledoc """
  Full FuXi-Linear model port, using the semantic-ID structure.
  ...
  **Memory:** FuXi-Linear uses linear attention / retention: the recurrence S_t = decay·S_{t-1} + k^T v
  ...
  def forward_hidden(batch_token_ids, batch_aux, embed_mask, params, opts) do
    ...
    # semantic ID: token embed + aux encoder
  ...
  defp apply_aux_encoder(batch_aux, embed_mask, params) do
    ...
    if w == nil, do: raise("FuXi requires ae.* params (semantic ID)")
  ...
  @doc """
  Build full model params: FuXi blocks + semantic ID (wte, ae.*, pred_head, ln_f).
  ...
  Use with existing the semantic-ID wte/ae/pred_head. Keys match forward/4 expectations.
  """
