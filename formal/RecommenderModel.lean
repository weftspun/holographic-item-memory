...
* **ResidualFSQ semantic-ID codec (upstream contract)** — the `multimodal-semantic-ids` Python
  pipeline that produces the IDs this library consumes (`Recommender.Memory` validates 3 tokens in
...
* **HRR phase algebra on the generation grid** — `lib/recommender/hrr.ex`. Atoms are SHA-256 uint16 values
...
* **Cleanup (nearest-item) transition recall** — `lib/recommender/memory.ex`. A stored `a → b` transition is
...
/-- Codes per stage `= 8^4` (`Recommender.Memory.codebook_size`). -/
def codebook : Nat := 4096

/-- Residual quantizer stages = tokens per item (`Recommender.Memory.tokens_per_item`). -/
...
/-! ## HRR phase algebra on the uint16 grid (`lib/recommender/hrr.ex`) -/
...
/-- Retrieval is exact on the grid: `unbind (bind a b) b = a`. The float
    implementation of `Recommender.HRR` only adds f64 representation noise on top. -/
...
/-- Budgeted cleanup: unbind the previous item's vector from the trace, then scan
    ...
    `Recommender.Memory.recommend`'s cleanup against the item map. -/
