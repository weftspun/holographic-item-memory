defmodule Holo.Core.MemoryTest do
  ...
  describe "semantic IDs" do
    test "contract constants match the multimodal-semantic-ids decision record" do
      assert Memory.tokens_per_item() == 4
      assert Memory.codebook_size() == 4096
    end

    test "valid_id?" do
      assert Memory.valid_id?([0, 4095, 17, 42])
      refute Memory.valid_id?([0, 4096, 17, 42])
      refute Memory.valid_id?([0, 17])
      refute Memory.valid_id?([0, 1, 2, -1])
      refute Memory.valid_id?(nil)
    end

    test "item_vector is deterministic" do
      a = Memory.item_vector([17, 900, 3, 5], 256)
      b = Memory.item_vector([17, 900, 3, 5], 256)
    ...
    test "item_vector rejects invalid ids" do
      ...
      assert_raise ArgumentError, fn -> Memory.item_vector([1, 2, 3, 4096], 64) end
    end

    test "items sharing semantic-id tokens are more similar than disjoint items" do
      dim = 1024
      base = Memory.item_vector([17, 900, 3, 5], dim)
      near = Memory.item_vector([17, 900, 3, 44], dim)
      far = Memory.item_vector([2000, 31, 999, 7], dim)
  ...
  test "content recall ranks an id-sharing item highly (zero-shot, no training)" do
    mem =
      loaded_memory()
      # twin shares the three coarse tokens of item-0 ([13, 7, 3, 5] vs [13, 7, 3, 999])
      |> Memory.add_item("twin-of-0", [13, 7, 3, 999])
  ...
  test "transition recall: an observed a -> b transition ranks b first" do
    ...
    # Query a source-only item: HRR binding is commutative (HoloModel.bind_comm),
    # so an item that is both a predecessor and a successor would recall both
    # neighbours ambiguously. A single a -> b transition is unambiguous.
    session = ["item-11", "item-27"]
    mem = Memory.observe(mem, session)
    assert mem.n_transitions == 1
  ...
  test "zero-shot: an item added after observations is immediately recommendable" do
    ...
    [t0, t1, _t2, _t3] = catalog() |> Enum.find(fn {id, _} -> id == "item-8" end) |> elem(1)
    mem = Memory.add_item(mem, "new-arrival", [t0, t1, 42, 7])
