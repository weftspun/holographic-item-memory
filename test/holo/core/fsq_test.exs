...
defmodule Holo.Core.FSQTest do
  use ExUnit.Case, async: true

  alias Holo.Core.FSQ

  # Pure-Elixir mirror of HoloModel.stageIndex (basis [1,8,64,512]).
  defp stage_index([c0, c1, c2, c3]), do: c0 + c1 * 8 + c2 * 64 + c3 * 512

  @all_digits for d0 <- 0..7, d1 <- 0..7, d2 <- 0..7, d3 <- 0..7, do: [d0, d1, d2, d3]

  describe "constants match the residual contract / HoloModel.lean" do
  ...
  describe "codes_to_indices / stage_roundtrip / stage_injective (exhaustive)" do
    test "every one of the 4096 digit tuples packs to HoloModel.stageIndex" do
